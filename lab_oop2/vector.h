...
#include "m_vector_const_iterator.h"
...
public:
    ...
    using ConstIterator = m_vector_const_iterator<T>;
    ...
    m_vector<T>& operator=(const m_vector<T>& other) {
        if (this != &other) {
            delete[] data;
            length = other.length;
    ...
    Iterator iterator_begin() { return Iterator(data); }
    Iterator iterator_end() { return Iterator(data + length); }

    ConstIterator iterator_begin() const { return ConstIterator(data); }
    ConstIterator iterator_end() const { return ConstIterator(data + length); }
};
